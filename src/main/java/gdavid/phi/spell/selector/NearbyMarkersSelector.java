...
public class NearbyMarkersSelector extends PieceSelector {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		Vector3 positionVal = getParamValueOrDefault(context, position, Vector3.fromVec3d(context.focalPoint.getPositionVec()));
		...
		AxisAlignedBB boundingBox = AxisAlignedBB.withSizeAtOrigin(radiusVal, radiusVal, radiusVal).offset(positionVal.toVec3D());
		boundingBox = boundingBox.intersect(AxisAlignedBB.withSizeAtOrigin(SpellContext.MAX_DISTANCE * 2, SpellContext.MAX_DISTANCE * 2, SpellContext.MAX_DISTANCE * 2).offset(context.focalPoint.getPositionVec()));
		return EntityListWrapper.make(context.focalPoint.getEntityWorld().getEntitiesWithinAABB(MarkerEntity.class, boundingBox));
	}
